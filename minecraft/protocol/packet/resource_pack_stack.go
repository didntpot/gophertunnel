...
// ResourcePackStack is sent by the server to send the order in which resource packs should be applied
...
type ResourcePackStack struct {
	...
	// join, or quit entirely.
	TexturePackRequired bool
	// ResourcePacks is a list of resource packs that the client needs to download before joining the server.
	// The order of these packs specifies the order that they are applied in on the client side. The
	// first in the list will be applied first.
	ResourcePacks []protocol.StackResourcePack
...
func (pk *ResourcePackStack) Marshal(io protocol.IO) {
	...
	protocol.Slice(io, &pk.ResourcePacks)
